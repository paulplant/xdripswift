    ...
    private func ensureActivity(contentState: XDripWidgetAttributes.ContentState, forceRestart: Bool = false) async {
        ...
        if forceRestart {
            ...
            if activityAge >= minimumForceRestartAge {
                ...
                trace("in ensureActivity, forceRestart triggered (existing Live Activity age: %{public}@ hours)", log: log, category: ConstantsLog.categoryLiveActivityManager, type: .info, activityAgeString)
                return
            } else {
                let activityAgeString = activityAge < 3600 ? activityAge.minutes.round(toDecimalPlaces: 1).description + " minutes" : activityAge.hours.round(toDecimalPlaces: 1).description + " hours"
                trace("in ensureActivity, forceRestart ignored (existing Live Activity age: %{public}@ seconds)", log: log, category: ConstantsLog.categoryLiveActivityManager, type: .info, activityAgeString)
            }
        }
