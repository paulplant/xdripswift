...
final class BluetoothSignalStrengthTests: XCTestCase {
    ...
    func testGaugeExpandsWithThreeDBmMarginAndResetsOnNewVisit() {
        var gauge = BluetoothSignalStrengthGauge()
        XCTAssertEqual(gauge.lowerBound, -120)
        XCTAssertEqual(gauge.upperBound, -80)
        gauge.observe(BluetoothSignalStrength(rssi: -78))
        XCTAssertEqual(gauge.upperBound, -75)
        XCTAssertEqual(gauge.position(for: -78), 42.0 / 45.0)
        gauge.observe(BluetoothSignalStrength(rssi: -122))
        XCTAssertEqual(gauge.lowerBound, -125)
        XCTAssertEqual(gauge.position(for: -122), 3.0 / 50.0)

        // Values within the padded range must not keep pushing the limits outwards.
        ...
        XCTAssertEqual(gauge.upperBound, -71)
        gauge.observe(BluetoothSignalStrength(rssi: -126))
        XCTAssertEqual(gauge.lowerBound, -129)
        XCTAssertEqual(gauge.position(for: -126), 3.0 / 58.0)
    ...
    func testGaugeRemembersActualExtremesAndResetsBothOnNewVisit() {
        ...
        XCTAssertEqual(gauge.upperBound, -67)
        XCTAssertEqual(gauge.position(for: gauge.minimumRSSI!), 3.0 / 61.0)
        XCTAssertEqual(gauge.position(for: gauge.maximumRSSI!), 58.0 / 61.0)
        gauge = BluetoothSignalStrengthGauge()
        XCTAssertNil(gauge.minimumRSSI)
        XCTAssertNil(gauge.maximumRSSI)
        gauge.observe(BluetoothSignalStrength(rssi: -95))
