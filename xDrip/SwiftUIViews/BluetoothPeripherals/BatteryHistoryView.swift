...
struct BatteryHistoryView: View {
    ...
    @ViewBuilder private func batteryValuePill(for point: BatteryHistoryPoint) -> some View {
        if let reading = batteryReading(for: point) {
            // Dexcom histories use Voltage B as their dominant value, while percentage devices
            // retain the generic Battery label used throughout their peripheral presentation.
